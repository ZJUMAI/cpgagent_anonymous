"""Pathology patch ROI selection from cached CONCH prompt scores."""
...
DEFAULT_PROMPT_IDS = ("tumor", "tumor_nests")
...
def run_conch_patch_roi(
    ...
    *,
    ...
    """Select and export patch ROIs for one pathology case."""
    ...
    overview_path = output_dir / "wsi_overview.png"
    wsi_path = _resolve_wsi_path(
        case_dir,
        case_yaml,
        str(scores.get("stem") or slide_dir.name),
    )
    overview_warning = _write_wsi_overview(wsi_path, overview_path)
    if overview_path.is_file():
        artifacts.append(("image", "wsi_overview", overview_path))
    elif overview_warning:
    ...
    metadata_path = output_dir / "conch_patch_roi_metadata.json"
    write_json(metadata_path, metadata)
    artifacts.append(("json", "roi_metadata", metadata_path))

    findings = {
        "summary": (
            f"Selected {len(selected)} pathology patch ROI image(s) from "
...
def resolve_conch_roi_slide_dir(
    ...
    *,
    ...
    raise FileNotFoundError(
        f"No slide CONCH ROI packet was found for case {case_id} under {searched}. {detail}"
    )
...
def _candidate_conch_roi_roots(
    ...
    roots.extend(
        [
            pathology_dir / "roi_256",
            pathology_dir / "roi_512",
            pathology_dir / "conch_roi",
        ]
    )
    unique: list[Path] = []
    seen: set[Path] = set()
...
def _path_from_uri(value: str) -> Path:
    if value.startswith("mock://"):
        raise ValueError(f"Mock URI cannot be used by pathology CONCH ROI skill: {value}")
...
def _write_wsi_overview(wsi_path: Path | None, destination: Path) -> str | None:
    """Write a low-resolution WSI overview PNG when the source slide is available."""

    if wsi_path is None:
        return "No source WSI was found; wsi_overview artifact was not generated."
    destination.parent.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                scale = min(1600 / max(width, 1), 1600 / max(height, 1), 1.0)
                size = (max(1, int(width * scale)), max(1, int(height * scale)))
                overview = slide.get_thumbnail(size).convert("RGB")
                overview.save(destination)
                return None
            finally:
                slide.close()
            ...
            return (
                "Could not generate WSI overview from "
                f"{wsi_path}: Pillow={pillow_exc}; OpenSlide={openslide_exc}"
            )
...
def _resolve_wsi_path(
    ...
    configured = _pathology_path_from_case_yaml(case_yaml, "wsi_dir")
    if configured is not None:
        candidates.append(configured)
    candidates.append(case_dir / "pathology" / "wsi")

    for wsi_dir in candidates:
        if not wsi_dir.is_dir():
            continue
        matches = sorted(wsi_dir.glob(f"{slide_stem}*"))
        if matches:
            return matches[0]
        files = sorted(path for path in wsi_dir.iterdir() if path.is_file())
        if len(files) == 1:
            return files[0]
    return None
...
def copy_default_prompts(destination: Path) -> None:
    """Copy the bundled default prompt file for setup helpers."""

    source = SKILL_DIR / "prompts" / "luad_default.json"
