"""NPC pathology patch ROI selection from cached CONCH prompt scores."""
...
DEFAULT_PROMPT_IDS = ("undifferentiated_npc", "nested_tumor")
DEFAULT_TOP_K_PER_PROMPT = 3
MAX_TOP_K_PER_PROMPT = 10
METADATA_FILENAME = "npc_conch_patch_roi_metadata.json"
...
def run_npc_conch_patch_roi(
    ...
    *,
...
) -> NpcConchPatchRoiOutput:
    """Select and export NPC pathology patch ROIs for one case."""
    ...
    findings = {
        "summary": (
            f"Selected {len(selected)} NPC pathology patch ROI image(s) from "
...
def _path_from_uri(value: str) -> Path:
    if value.startswith("mock://"):
        raise ValueError(
            f"Mock URI cannot be used by NPC pathology CONCH ROI skill: {value}"
        )
...
def copy_default_prompts(destination: Path) -> None:
    """Copy the bundled NPC default prompt file for setup helpers."""

    source = SKILL_DIR / "prompts" / "npc_default.json"
